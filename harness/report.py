...
n = len(rows)
graded = [r for r in rows if r.get("verdict") in ("pass", "fail")]
...
total_in = sum(r.get("input_tokens", 0) + r.get("judge_input_tokens", 0) for r in rows)
total_out = sum(r.get("output_tokens", 0) + r.get("judge_output_tokens", 0) for r in rows)
...
"pass_rate_pct": round(100 * n_pass / len(graded), 1) if graded else None,
"mean_score": round(statistics.mean(scores), 2) if scores else None,
"score_distribution": {str(s): scores.count(s) for s in sorted(set(scores))},
"cost_usd": {
"explainer": round(explainer_cost, 4),
"judge": round(judge_cost, 4),
"total": round(explainer_cost + judge_cost, 4),
"per_1k_items_explainer_only": round(explainer_cost / max(n, 1) * 1000, 2),
"projected_at_100x_items": round((explainer_cost + judge_cost) * 100, 2),
},
"latency_s": {
"mean": round(statistics.mean(latencies), 3) if latencies else None,
"p95": round(sorted(latencies)[int(0.95 * len(latencies))], 3) if latencies else None,
},
